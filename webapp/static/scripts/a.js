...
I need help with making an upgraded UI and improving the user flow of my custom editor application. You see, I have an editor that allows the user to select, modify, and save an API, JSON, and Python functions that process GeoJSON data. There are a few current parts of this window that I have and I want to change aspects and the layout of the user flow.

The point of this application is for a user to craft their own custom datasets from ArcGIS and then those datasets will be read by my application which renders them onto a plotly map. I have all of the functionality for the map, but I need help improving the user flow for creating 3 main parts of this dataset.

What currently happens is that the user selects the dataset that they want to modify by either choosing one of the provided datasets that I have pre-selected, or they can paste their own API that will ingest their GeoJSON file. This is the first part that the user does for selecting a dataset, and I need to change where this happens. Right now, my custom editor has 2 windows on one HTML page, the left side, which contains all of the options, and the right side which is a preview window so the user can preview the output of what they are getting based on their selections. In this left side window contains all of the options including the "API Options" box, where the dataset is currently selected. There are two dropdowns for selecting the API, the first one is the "Source Type" which asks the user if they want to use one of the provided APIs or use one of their own. If they select the provided API option, then below that is another dropdown that allows the user to select an API which then populates the API URL input box with the API selected. Directly below this API URL input box, there is a "Load Fields" button when pressed, it loads in the fields from the fields from the GeoJSON into the "API Fields" subcategory. This subcategory within this API options category which has "API Fields" and it is populated with all of the fields from the GeoJSON loaded in. This API fields have three columns, one for the name of the field, the next is a prediction as if it is a qualitative or quantitative property, and a checkbox to select if the user wants to process the field(s) in that dataset. Below the API Fields subcategory is another subcategory of Spatial Inputs which contains another dropdown, and then below that subcategory is another subcategory called Output Options which contains some more options. And the below that is an input box of Number of Objects to Load: with a number as the input that loads the objects.

This is the first thing that I want to change, because it is not clear that selecting the API is what is actually selecting the dataset. Instead of having the dataset choosing UI in the "API Options" category, I want to have this functionality to have its own category, what all of the backend functionality remains the same. I want all of this functionality to still remain in the
