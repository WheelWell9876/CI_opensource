...
def calculate_dataset_overall(dataset_json):
    """
    Given a dataset JSON (with qualitativeFields), update each qualitative field’s properties overall values.
    """
...
def calculate_dataset_grade(dataset_json):
    """
    Compute a simple dataset grade as the average of the field grades in the 'summaryOfGrades' dict.
    """
...
def create_json_object(data, analysis=None):
    ...
    If category information is present, create a category JSON; otherwise, create a dataset JSON.
    """
...
def export_json(data, folder, filename):
    """
